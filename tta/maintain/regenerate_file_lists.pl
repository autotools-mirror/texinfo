...
sub wanted  {
  if ((/\.pl$/ and $File::Find::dir =~ m:^t/results/[^/]+:)
      # Listing each file lead to an "Argument list too long" error
      # for the shell in make dist since 2023.
      # In 2026, there is an "Argument list too long" error even with
      # directories when the files and directories are listed in EXTRA_DIST.
