...
%
\def\texinfoversion{2026-09-26.20}
%
...
\parseargdef\appendix{\apphead0{#1}} % normally calls appendixzzz
%
\def\appendixzzz#1{%
  \global\secno=0 \global\subsecno=0 \global\subsubsecno=0
...
\parseargdef\unnumbered{\unnmhead0{#1}}
\def\unnumberedzzz#1{%
...
% Until the next @node, top-level section command, @part or @bye command,
% divert output to a box that is not output.
\def\ignorenode{\setbox\dummybox\vbox\bgroup
  % redefine top-level commands to close the dummy
  % box and restart processing.
  \def\restartat##1{\def##1{\egroup##1}}%
  \restartat\part
  \restartat\node
  \restartat\chapter
  \restartat\centerchap
  \restartat\unnumbered
  \restartat\appendix
  \ignorenodebye
}
...
\def\setrefshadow#1#2{%
  % For PDF, output the target with \pdfdest.
  % Temporarily suppress pdfTeX warning for trying to output two targets
  % with the same name.
  \pdfsuppresswarningdupdest1
  \pdfmkdest{\pdfshadowprefix#2}%
  \pdfsuppresswarningdupdest0
  \dosetref{#2}{#1}{#2}{xrshad}%
}

% Primitive introduced in pdfTeX 1.40.13.  Supply dummy definition for
% older versions or when not running with pdfTeX.
\ifx\pdfsuppresswarningdupdest\thisisundefined
  \def\pdfsuppresswarningdupdest#1{}%
\fi
...
\def\xrefX[#1,#2,#3,#4,#5,#6]{\begingroup
  ...
  \ifdim \wd\printedmanualbox = 0pt
    ...
    \else\expandafter\ifnum\countA = 2
      \txiwarning{ambiguous xref to #1}%
      \startxreflink{}{}%
    \else
      \startxreflink{#1}{}%
    \fi\fi
...
% count register sequence in #2 to:
%  1 if they are active
%  0 if they are not
%  2 if multiple definitions were made
\def\checkshadowactive#1#2{%
  \requireauxfile
  {%
    \xrdefctxt
    \expandafter\global\expandafter\let\expandafter\tmp\csname XR#1-title\endcsname
  }%
  \ifx\tmp\relax
    % a real definition was not made
    {%
      \xrdefctxt
    ...
    \else\ifx\tmp\empty
      #2=2 % shadow definitions were disabled
    \else
